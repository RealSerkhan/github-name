...
public class MergeApp3 {
  ...
  private static List<Apple> merge(List<Apple> data1, List<Apple> data2, Comparator<Apple> ac) {
    ArrayList<Apple> data3 = new ArrayList<>();
    int idx1 = 0;
    int idx2 = 0;

    while (idx1 < data1.size() && idx2 < data2.size()) {
      if (ac.compare(data1.get(idx1), data2.get(idx2)) < 0) data3.add(
              data1.get(idx1++)
      );
      else data3.add(
              data2.get(idx2++)
